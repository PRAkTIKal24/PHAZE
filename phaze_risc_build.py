...
def build_guest_programs(dataset_name="mnist"):
    ...
    try:
        ...
        from phaze.src.risc_zero_codegen import (
            RiscZeroArchitectureRegistry,
            RiscZeroBuildManager,
        )
        ...
        try:
            ...
            print(
                f"📊 Dataset info: {dataset_config['input_size']} inputs → {dataset_config['output_size']} classes"
            )
        ...
        if success_count == 0:
            ...
            print(
                "🔗 Release symlinks automatically created for RISC Zero backend compatibility"
            )
...
def list_architectures():
    ...
    try:
        ...
        for arch_key in sorted(architectures):
            ...
            if arch_info:
                print(
                    f"    Input: {arch_info['input_size']}, Output: {arch_info['output_size']}"
                )
...
def scan_factory(dataset_name="mnist"):
    ...
    try:
        ...
        try:
            ...
            print(
                f"📊 Input: {dataset_config['input_size']} ({dataset_config['input_channels']}×{dataset_config['spatial_size']}×{dataset_config['spatial_size']})"
            )
        ...
        if registered_count < total_combinations:
            print(
                f"💡 Run 'phaze-risc-build build --dataset {dataset_name}' to register missing architectures"
            )
...
def list_datasets():
    ...
    try:
        ...
        for name in list_supported_datasets():
            ...
            print(
                f"    Input: {config.input_size} ({config.input_channels}×{config.spatial_size}×{config.spatial_size})"
            )
