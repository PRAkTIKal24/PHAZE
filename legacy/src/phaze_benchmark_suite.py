...
class PHAZEBenchmarkResults:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if "frameworks_to_test" in config_dict:
            config_dict["frameworks_to_test"] = [f.value for f in self.config.frameworks_to_test]
...
class PHAZEBenchmarkSuite:
    ...
    async def run_full_benchmark_suite(self, zkml_config: Dict[str, Any], crypto_config: Dict[str, Any]) -> Dict[str, Any]:
        """Compatibility method that wraps run_full_benchmark with test-expected interface."""
        # Modify config based on test parameters
        if zkml_config:
            self.config.num_iterations = zkml_config.get("num_trials", self.config.num_iterations)
            self.config.zkml_iterations = zkml_config.get("num_trials", self.config.zkml_iterations)
            
        if crypto_config:
            self.config.crypto_iterations = crypto_config.get("num_trials", self.config.crypto_iterations)
            ...
            "zkml_results": [
                {
                    ...
                    "total_time": (metrics.setup_time_ms or 0) + (metrics.proving_time_ms or 0) + (metrics.verification_time_ms or 0),
            ...
            "crypto_results": [
                {
                    ...
                    "success": True,
                    "execution_time": metrics.get("avg_time_ms", 0) / 1000,  # Convert to seconds
            ...
            "summary": self._generate_summary(
                ...
                [
                    type('BenchmarkResult', (), {
                        'framework': framework,
                        'success': metrics.success,
                        'total_time': ((metrics.setup_time_ms or 0) + (metrics.proving_time_ms or 0) + (metrics.verification_time_ms or 0)) / 1000,
                        'setup_time': (metrics.setup_time_ms or 0) / 1000,
                        'proof_time': (metrics.proving_time_ms or 0) / 1000, 
                        'verification_time': (metrics.verification_time_ms or 0) / 1000,
                        'memory_usage_mb': metrics.memory_usage_mb or 0
                    })()
                    for framework, metrics in results.zkml_results.items()
                ],
    ...
    def _generate_summary(self, zkml_results: List, crypto_results: List) -> Dict[str, Any]:
        ...
        if successful_zkml:
            if hasattr(successful_zkml[0], 'setup_time'):
                zkml_summary["avg_setup_time"] = np.mean([r.setup_time for r in successful_zkml])
            if hasattr(successful_zkml[0], 'proof_time'):  
                zkml_summary["avg_proof_time"] = np.mean([r.proof_time for r in successful_zkml])
            if hasattr(successful_zkml[0], 'verification_time'):
                zkml_summary["avg_verification_time"] = np.mean([r.verification_time for r in successful_zkml])
            if hasattr(successful_zkml[0], 'memory_usage_mb'):
                zkml_summary["avg_memory_usage"] = np.mean([r.memory_usage_mb for r in successful_zkml])
        ...
        return {
            ...
            "overall_summary": {
                ...
                "success_rate": (len(successful_zkml) + len(successful_crypto)) / (len(zkml_results) + len(crypto_results)) if (zkml_results or crypto_results) else 0
            }
        }
