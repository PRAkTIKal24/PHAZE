...
async def example_cifar10():
    ...
    from phaze.src.model_architectures import ModelComplexity, PHAZEModelFactory

    # Get CIFAR-10 configuration
    cifar10_config = get_dataset_config("cifar10")
    print(f"Dataset: {cifar10_config.description}")
    print(
        f"Input shape: {cifar10_config.input_channels}×{cifar10_config.spatial_size}×{cifar10_config.spatial_size}"
    )
    ...
    sample_input = torch.randn(
        1,
        cifar10_config.input_channels,
        cifar10_config.spatial_size,
        cifar10_config.spatial_size,
    )
...
async def example_custom_dataset():
    ...
    from phaze.src.dataset_config import (
    ...
    print("✅ Custom dataset configuration valid")
    print(f"Dataset: {custom_config.description}")
    print(
        f"Input: {custom_config.input_size} ({custom_config.input_channels}×{custom_config.spatial_size}×{custom_config.spatial_size})"
    )
...
def example_dataset_comparison():
    ...
    print(
        f"{'Dataset':<15} {'Input Size':<10} {'Classes':<8} {'Channels':<8} {'Spatial':<8}"
    )
    print("-" * 60)
    ...
    for name in datasets:
        ...
        print(
            f"{name:<15} {config.input_size:<10} {config.output_size:<8} "
