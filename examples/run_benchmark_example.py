...
from phaze.src.comprehensive_benchmark import run_phaze_benchmarks, ComprehensiveBenchmarkSuite
...
async def run_standard_benchmarks(output_dir, quick_mode=True):
    ...
    try:
        ...
        if "frameworks_tested" in result["results"]["summary"]["zkml_summary"]:
            if "risc_zero" not in result["results"]["summary"]["zkml_summary"]["frameworks_tested"]:
                result["results"]["summary"]["zkml_summary"]["frameworks_tested"].append("risc_zero")
...
async def run_risc_zero_standalone(output_dir):
    ...
    print(f"✅ Proof generated: {proof['framework']} proof with {len(proof['proof_data'])} bytes")
...
async def run_risc_zero_benchmark(output_dir):
    ...
    try:
        ...
        for architecture in architectures:
            for complexity in complexities:
                for input_size in input_sizes:
                    print(f"🔄 Benchmarking: {architecture}/{complexity}, input_size={input_size}")
        ...
        print(f"✅ Successful tests: {len(successful)}/{len(all_results)} ({len(successful)/len(all_results):.1%})")
        ...
        if successful:
            ...
            avg_verify_time = sum(r.verification_time for r in successful) / len(successful)
...
async def run_extended_benchmark(output_dir):
    ...
    try:
        ...
        result["results"]["zkml_results"].extend([r.__dict__ for r in risc_zero_results])
...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description="PHAZE Benchmarking Examples"
    )
    parser.add_argument(
        "--mode",
    ...
    parser.add_argument(
        "--quick",
        action="store_true",
        help="Run in quick mode with limited tests"
    )
