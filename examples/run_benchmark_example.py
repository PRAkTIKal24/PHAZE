...
async def main():
    ...
    try:
        ...
        if zkml_summary:
            ...
            print(f"  - Frameworks tested: {', '.join(zkml_summary.get('frameworks_tested', []))}")
            print(f"  - Success rate: {zkml_summary.get('success_rate', 0):.1%}")
            print(f"  - Avg proof time: {zkml_summary.get('avg_proof_time', 0):.3f}s")
            print(f"  - Avg memory usage: {zkml_summary.get('avg_memory_usage', 0):.1f}MB")
            print()
        ...
        if crypto_summary:
            ...
            print(f"  - Primitives tested: {', '.join(crypto_summary.get('primitives_tested', []))}")
            print(f"  - Success rate: {crypto_summary.get('success_rate', 0):.1%}")
            print(f"  - Avg throughput: {crypto_summary.get('avg_throughput', 0):.1f} ops/sec")
            print()
        ...
        if zkml_results:
            ...
            if successful_zkml:
                ...
                print(f"zkML Test: {sample.get('framework', 'Unknown')} - {sample.get('architecture', 'Unknown')}")
        ...
        if crypto_results:
            ...
            if successful_crypto:
                ...
                print(f"Crypto Test: {sample.get('primitive_name', 'Unknown')} - {sample.get('operation', 'Unknown')}")
                print(f"  Execution: {sample.get('execution_time', 0):.6f}s")
                print(f"  Throughput: {sample.get('throughput_ops_per_sec', 0):.1f} ops/sec")
