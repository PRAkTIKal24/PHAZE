...
class PHAZEBenchmarkSuite:
    ...
    def _register_zkml_backends(self):
        ...
        for framework in self.config.frameworks_to_test:
            try:
                backend = create_backend(framework, self.m_full, f"benchmark_{framework.value}")
    ...
    async def benchmark_early_exit_model(self) -> Dict[str, float]:
        ...
        return {
            ...
            "throughput_inferences_per_sec": 1000 / avg_time_ms if avg_time_ms > 0 else 0
        }
    ...
    async def benchmark_crypto_primitives(self) -> Dict[str, Dict[str, float]]:
        ...
        return self.crypto_benchmark.benchmark_all_primitives(self.config.crypto_iterations)
    ...
    async def run_full_benchmark(self) -> PHAZEBenchmarkResults:
        ...
        if self.config.save_results:
            ...
            filepath = os.path.join(
                ...
                f"phaze_benchmark_{timestamp_str}.json"
            )
    ...
    def generate_report(self, results: PHAZEBenchmarkResults) -> str:
        ...
        if successful_zkml:
            for framework, metrics in successful_zkml.items():
                ...
                report += f"    Verification Time: {metrics.verification_time_ms:.2f} ms\n"
        ...
        m_early_throughput = results.m_early_results.get("throughput_inferences_per_sec", 0)
        m_full_throughput = results.m_full_results.get("throughput_inferences_per_sec", 0)
        ...
        if successful_zkml:
            fastest_zkml = min(successful_zkml.items(), key=lambda x: x[1].proving_time_ms)
        ...
        if avg_pipeline > 0:
            ...
            report += f"  Pipeline Throughput: {pipeline_throughput:.2f} pipelines/sec\n"
