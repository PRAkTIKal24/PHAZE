...
from .plot_registry import get_global_registry, PlotRegistry
...
class PHAZEPlotSuite:
    ...
    def get_available_plot_types(self, data: Optional[Dict[str, Any]] = None) -> List[str]:
    ...
    def generate_plots(
        ...
        if not plot_types:
            ...
            return {'plots': {}, 'metadata': {'plot_types': [], 'error': 'No compatible plot types'}}
        
        # Validate data
        if not self._validate_data(data):
            logger.error("Data validation failed")
            return {'plots': {}, 'metadata': {'plot_types': [], 'error': 'Data validation failed'}}
        ...
        logger.info(f"Plot generation completed. Generated {len(results['plots'])} plot types")
        return results
    ...
    def generate_comparative_plots(
        ...
        comparative_types = [pt for pt in self.registry.list_plot_types() if 'comparative' in pt]
        
        if not comparative_types:
            logger.warning("No comparative plotter found")
            return {'plots': {}, 'metadata': {'error': 'No comparative plotter available'}}
        ...
        if plots:
            ...
            for plot_type, plot_data in plots.items():
                ...
                if error:
                    ...
                    status = f"✅ **Success:** {num_plots} plots, {num_files} files saved"
