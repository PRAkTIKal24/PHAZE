...
class ZKMLPlotter(BasePlotter):
    ...
    including proof generation time, verification time, memory usage, and comparative analysis.
    """
    ...
    def _organize_zkml_data(self, zkml_results: List[Dict[str, Any]]) -> Dict[str, Dict[str, List[Any]]]:
        ...
        for result in zkml_results:
            ...
            if result.get("success", False):
                organized_data[framework]["complexities"].append(result.get("complexity", "unknown"))
                organized_data[framework]["input_sizes"].append(result.get("input_size", 0))
                organized_data[framework]["model_parameters"].append(result.get("model_parameters", 0))
                organized_data[framework]["setup_times"].append(result.get("setup_time", 0))
                organized_data[framework]["proof_times"].append(result.get("proof_time", 0))
                organized_data[framework]["verification_times"].append(result.get("verification_time", 0))
                organized_data[framework]["memory_usage"].append(result.get("memory_usage_mb", 0))
                organized_data[framework]["proof_sizes"].append(result.get("proof_size_bytes", 0))
                organized_data[framework]["success_flags"].append(True)
                organized_data[framework]["architectures"].append(result.get("architecture", "unknown"))
                organized_data[framework]["total_times"].append(result.get("total_time", 0))
    ...
    def _plot_proof_generation_time(self, data: Dict[str, Dict[str, List[Any]]]) -> matplotlib.figure.Figure:
        ...
        for i, (framework, framework_data) in enumerate(data.items()):
            ...
            complexity_groups = {}
            param_groups = {}
            for complexity, time, params in zip(complexities, proof_times, model_params):
    ...
    def _plot_verification_time(self, data: Dict[str, Dict[str, List[Any]]]) -> matplotlib.figure.Figure:
        ...
        for i, (framework, framework_data) in enumerate(data.items()):
            ...
            complexity_groups = {}
            param_groups = {}
            for complexity, time, params in zip(complexities, verification_times, model_params):
                if complexity not in complexity_groups:
                    complexity_groups[complexity] = []
                    param_groups[complexity] = []
                complexity_groups[complexity].append(time)
    ...
    def _plot_memory_usage(self, data: Dict[str, Dict[str, List[Any]]]) -> matplotlib.figure.Figure:
        ...
        for i, (framework, framework_data) in enumerate(data.items()):
            ...
            complexity_groups = {}
            param_groups = {}
            for complexity, mem, params in zip(complexities, memory_usage, model_params):
                if complexity not in complexity_groups:
                    complexity_groups[complexity] = []
                    param_groups[complexity] = []
    ...
    def _plot_proof_size_analysis(self, data: Dict[str, Dict[str, List[Any]]]) -> matplotlib.figure.Figure:
        ...
        for i, (framework, framework_data) in enumerate(data.items()):
            ...
            valid_data = [(p, s) for p, s in zip(model_params, proof_sizes) if s > 0]
            if not valid_data:
                continue
            
            params, sizes = zip(*valid_data)
    ...
    def _plot_scaling_analysis(self, data: Dict[str, Dict[str, List[Any]]]) -> matplotlib.figure.Figure:
        ...
        for i, (framework, framework_data) in enumerate(data.items()):
            ...
            sorted_data = sorted(zip(model_params, proof_times, verification_times))
            params, p_times, v_times = zip(*sorted_data)
