...
class ConvolutionalEarlyExitModel(PHAZEModelInterface):
    ...
    def __init__(self, input_channels: int = 3, input_size: int = 32, output_size: int = 10, 
                 complexity: ModelComplexity = ModelComplexity.LIGHT):
        super().__init__(input_channels * input_size * input_size, output_size, complexity)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if len(x.shape) == 2:
            x = x.view(-1, self.input_channels, self.input_spatial_size, self.input_spatial_size)
...
class TransformerEarlyExitModel(PHAZEModelInterface):
    ...
    def __init__(self, input_size: int = 512, output_size: int = 10, 
...
class MultiExitModel(PHAZEModelInterface):
    ...
    def __init__(self, input_size: int = 10, output_size: int = 5, 
        ...
        for i in range(len(self.layer_sizes) - 1):
            self.backbone_layers.append(nn.Linear(self.layer_sizes[i], self.layer_sizes[i + 1]))
    ...
    def forward(self, x: torch.Tensor, exit_layer: Optional[int] = None) -> torch.Tensor:
    ...
    def get_confidence(self, x: torch.Tensor, exit_layer: Optional[int] = None) -> torch.Tensor:
    ...
    def adaptive_forward(self, x: torch.Tensor, confidence_threshold: float = 0.8) -> Tuple[torch.Tensor, int]:
    ...
    def create_early_exit_model(
        ...
        **kwargs
        ...
        if architecture == "simple":
            ...
            return ConvolutionalEarlyExitModel(input_channels, spatial_size, output_size, complexity)
...
def create_simple_early_exit_model(input_size: int = 10, output_size: int = 5) -> SimpleEarlyExitModel:
    """Create a simple early-exit model."""
    return SimpleEarlyExitModel(input_size, output_size)


def create_simple_full_model(input_size: int = 10, output_size: int = 5) -> PHAZEModelInterface:
    """Create a simple full model."""
    return PHAZEModelFactory.create_full_model("simple", ModelComplexity.MEDIUM, input_size, output_size)
