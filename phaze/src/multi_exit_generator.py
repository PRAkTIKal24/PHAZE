...
class MultiExitGenerator:
    ...
    def analyze_model_layers(
        self, model: nn.Module, architecture: str
    ) -> Dict[str, Any]:
        ...
        if architecture == "simple":
            ...
            logger.warning(
                f"Unknown architecture {architecture}, using generic analysis"
            )
    ...
    def find_truncation_points(
        self, layer_info: Dict[str, Any], target_ratios: List[float]
        ...
        for target_ratio in target_ratios:
            ...
            logger.info(
                f"Target ratio {target_ratio:.2f} -> Layer {best_idx}, "
    ...
    def create_early_exit_model(
        ...
        if architecture == "simple":
            return self._create_simple_early_exit(
                source_model, truncation_layer, model_info
            )
        elif architecture == "conv":
            return self._create_conv_early_exit(
                source_model, truncation_layer, model_info
            )
        elif architecture == "transformer":
            return self._create_transformer_early_exit(
                source_model, truncation_layer, model_info
            )
        elif architecture == "multi_exit":
            return self._create_multi_exit_early_exit(
                source_model, truncation_layer, model_info
            )
        else:
            logger.warning(f"Unknown architecture {architecture}, using generic method")
            return self._create_generic_early_exit(
                source_model, truncation_layer, model_info
            )

    def _create_simple_early_exit(
        self, model: nn.Module, truncation_layer: int, model_info: Dict[str, Any]
    ) -> nn.Module:
    ...
    def _create_conv_early_exit(
        ...
        target_complexity = complexity_map.get(
            truncation_layer, ModelComplexity.MINIMAL
        )
    ...
    def _create_transformer_early_exit(
        self, model: nn.Module, truncation_layer: int, model_info: Dict[str, Any]
    ) -> nn.Module:
    ...
    def _create_multi_exit_early_exit(
        self, model: nn.Module, truncation_layer: int, model_info: Dict[str, Any]
    ) -> nn.Module:
    ...
    def _create_generic_early_exit(
        self, model: nn.Module, truncation_layer: int, model_info: Dict[str, Any]
    ) -> nn.Module:
    ...
    def generate_early_exit_variants(
        self,
        full_model: nn.Module,
        architecture: str,
        complexity: str,
        ...
        truncation_points = self.find_truncation_points(
            layer_info, self.config.model.early_exit_ratios
        )
        ...
        for i, (truncation_layer, actual_ratio) in enumerate(truncation_points):
            try:
                ...
                early_info = {
                    ...
                    - (early_params / model_info["parameters"]),
                    "model": early_model,
                ...
                logger.info(
                    f"  Parameters: {early_params:,} ({actual_ratio:.1%} of original)"
                )
    ...
    def select_largest_models(self, training_results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for _model_id, model_info in training_results.items():
            ...
            if (
                architecture not in largest_models
                or parameters > largest_models[architecture]["parameters"]
            ):
        ...
        for arch, model_info in largest_models.items():
            logger.info(
                f"  {arch}: {model_info['model_id']} ({model_info['parameters']:,} params)"
            )

        return largest_models

    def save_early_exit_model(
        self, early_info: Dict[str, Any], output_dir: Path
    ) -> Dict[str, str]:
        ...
        metadata = {
            k: v for k, v in early_info.items() if k != "model"
        }  # Exclude model object
...
def generate_early_exits_from_model(
    config: PHAZEConfig,
    full_model: nn.Module,
    architecture: str,
    complexity: str,
...
if __name__ == "__main__":
    ...
    for i, (layer, params) in enumerate(
        zip(layer_info["layers"], layer_info["layer_parameters"], strict=False)
    ):
