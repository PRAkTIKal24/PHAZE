...
class EnhancedZKMLBenchmark:
    ...
    async def benchmark_single_model(self, model: nn.Module, model_info: Dict[str, Any],
                                   framework: str = "ezkl") -> Dict[str, Any]:
        ...
        try:
            ...
            for i in range(self.config.experiment.benchmark_iterations):
                ...
                try:
                    if framework == "ezkl":
                        ...
                        proof, output = await self._generate_risc_zero_proof(zkml_system, sample_input)
            ...
            if proof_memory_stats:
                result['proof_memory'] = {
                    ...
                    'peak_increase_mb': np.mean([m['peak_increase_mb'] for m in proof_memory_stats])
                }

            logger.info(f"  Proof generation: {result['proof_time']:.2f}s avg "
            ...
            for i in range(self.config.experiment.benchmark_iterations):
                ...
                try:
                    if framework == "ezkl":
                        verified = await zkml_system.verify_proof(stored_proof, sample_input)
                    else:
                        verified = await self._verify_risc_zero_proof(zkml_system, stored_proof, sample_input)
            ...
            if verification_times:
                ...
                if verification_memory_stats:
                    result['verification_memory'] = {
                        'peak_mb': np.mean([m['peak_mb'] for m in verification_memory_stats]),
                        'peak_increase_mb': np.mean([m['peak_increase_mb'] for m in verification_memory_stats])
                    }

                logger.info(f"  Verification: {result['verification_time']:.2f}s avg "
                           f"({len(verification_times)} successful)")

            # Calculate total time and overall memory usage
            result['total_time'] = result['setup_time'] + result['proof_time'] + result['verification_time']
    ...
    async def _create_risc_zero_system(self, model: nn.Module, model_info: Dict[str, Any]):
    ...
    async def benchmark_multiple_models(self, models_info: List[Dict[str, Any]],
                                      frameworks: Optional[List[str]] = None) -> List[Dict[str, Any]]:
        ...
        logger.info(f"Starting batch benchmark: {len(models_info)} models, "
        ...
        for model_info in models_info:
            ...
            for framework in frameworks:
                try:
                    result = await self.benchmark_single_model(model, model_info, framework)
                    ...
                    logger.error(f"Failed to benchmark {model_info.get('model_id')} "
                               f"with {framework}: {e}")
    ...
    def get_benchmark_summary(self) -> Dict[str, Any]:
        ...
        for framework, results in framework_groups.items():
            ...
            summary['frameworks'][framework] = {
                ...
                'success_rate': len(results) / (len(results) +
                    len([f for f in self.failed_benchmarks if f['framework'] == framework]))
            }
        ...
        all_verification_times = [r['verification_time'] for r in self.benchmark_results]
        ...
        summary['overall_stats'] = {
            ...
            'verification_time_range': [np.min(all_verification_times), np.max(all_verification_times)]
        }
...
class MockRiscZeroSystem:
    ...
    async def verify_proof(self, proof, sample_input: torch.Tensor):
        ...
        return isinstance(proof, dict) and 'proof' in proof and 'risc_zero' in proof.get('framework', '')


# Convenience functions
async def benchmark_model_list(config: PHAZEConfig, models_info: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
