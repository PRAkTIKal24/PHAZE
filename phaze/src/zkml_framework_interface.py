...
class ZKMLBenchmarkRunner:
    ...
    async def benchmark_framework(
        ...
        try:
            ...
            for _ in range(num_iterations):
                ...
                if not is_valid:
                    metrics.error_message = f"Proof verification failed on iteration {_}"
                    return metrics
            
            # Calculate average metrics
            metrics.proving_time_ms = sum(proving_times) / len(proving_times)
            metrics.verification_time_ms = sum(verification_times) / len(verification_times)
    ...
    async def benchmark_all_frameworks(
        ...
        for framework in self.backends.keys():
            ...
            metrics = await self.benchmark_framework(framework, input_data, num_iterations)
...
class CryptographicPrimitiveBenchmark:
    ...
    def benchmark_primitive(self, name: str, num_iterations: int = 1000) -> Dict[str, float]:
    ...
    def benchmark_all_primitives(self, num_iterations: int = 1000) -> Dict[str, Dict[str, float]]:
