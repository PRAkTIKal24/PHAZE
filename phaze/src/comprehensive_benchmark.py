        ...
        for trial in range(num_trials):
            ...
            try:
                ...
                model = self.zkml_integration.create_and_register_model(
                    model_name, architecture, complexity, "full",
                    input_size=input_size, output_size=output_size
                )
                
                result.model_parameters = sum(p.numel() for p in model.parameters() if p.requires_grad)
                ...
                if framework_name == "ezkl":
                    proof, output = await self.zkml_integration.prove_inference(model_name, input_data)
                elif framework_name in ["groth16", "plonky", "halo"]:
                    backend = self.rust_manager.get_backend(framework_name)
                ...
                if framework_name == "ezkl":
                    is_valid = await self.zkml_integration.verify_inference(model_name, proof, input_data)
                elif framework_name in ["groth16", "plonky", "halo"]:
                    backend = self.rust_manager.get_backend(framework_name)
            ...
            for architecture in architectures:
                for complexity in complexities:
                    for input_size in input_sizes:
                        self.logger.info(f"Benchmarking {framework} with {architecture}/{complexity}, input_size={input_size}")
                        
                        results = await self.benchmark_framework(
                            framework, architecture, complexity, 
                            input_size, 5, num_trials
                        )
...
class CryptographicPrimitiveBenchmark:
    ...
    def benchmark_shamir_secret_sharing(
        ...
        for secret_size in secret_sizes:
            ...
            test_secret = np.random.randint(0, 256, secret_size, dtype=np.uint8).tobytes()
            ...
            if 'shares' in locals():
                ...
                for _ in range(num_trials):
                    try:
                        reconstructed = sss.reconstruct_secret(shares[:3])  # Use threshold number of shares
                    except Exception as e:
    ...
    def benchmark_rust_primitives(self) -> List[CryptoBenchmarkResult]:
        ...
        for field_size in field_sizes:
            try:
                ...
                benchmark_results = rust_manager.base_backend.benchmark_field_operations(field_size, 1000)
                ...
                for operation, time_ms in benchmark_results.items():
                    result = CryptoBenchmarkResult(
                        ...
                        memory_usage_mb=memory_usage,
                        throughput_ops_per_sec=1000.0 / (time_ms / 1000.0) if time_ms > 0 else 0,
                        success=True
                    )
                    results.append(result)
...
class ComprehensiveBenchmarkSuite:
    ...
    async def run_full_benchmark_suite(
        self,
        zkml_config: Dict[str, Any] = None,
        crypto_config: Dict[str, Any] = None
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            zkml_results = await self.zkml_benchmark.benchmark_all_frameworks(**zkml_config)
            ...
            rabin_results = self.crypto_benchmark.benchmark_rabin_fingerprint(
                crypto_config["rabin_input_sizes"], 
                crypto_config["num_trials"]
            )
            
            shamir_results = self.crypto_benchmark.benchmark_shamir_secret_sharing(
                crypto_config["shamir_secret_sizes"], 
                crypto_config["num_trials"]
            )
            ...
            results["summary"] = self._generate_summary(zkml_results, all_crypto_results)
    ...
    def _generate_summary(
        ...
        if zkml_results:
            ...
            summary["zkml_summary"] = {
                ...
                "success_rate": len(successful_zkml) / len(zkml_results) if zkml_results else 0,
                "avg_setup_time": np.mean([r.setup_time for r in successful_zkml]) if successful_zkml else 0,
                "avg_proof_time": np.mean([r.proof_time for r in successful_zkml]) if successful_zkml else 0,
                "avg_verification_time": np.mean([r.verification_time for r in successful_zkml]) if successful_zkml else 0,
                "avg_total_time": np.mean([r.total_time for r in successful_zkml]) if successful_zkml else 0,
                "avg_memory_usage": np.mean([r.memory_usage_mb for r in successful_zkml]) if successful_zkml else 0,
        ...
        if crypto_results:
            ...
            summary["crypto_summary"] = {
                ...
                "success_rate": len(successful_crypto) / len(crypto_results) if crypto_results else 0,
                "avg_execution_time": np.mean([r.execution_time for r in successful_crypto]) if successful_crypto else 0,
                "avg_throughput": np.mean([r.throughput_ops_per_sec for r in successful_crypto]) if successful_crypto else 0,
                "primitives_tested": list(set(r.primitive_name for r in crypto_results)),
                "operations_tested": list(set(r.operation for r in crypto_results))
            }
        
        # Overall summary
        total_tests = len(zkml_results) + len(crypto_results)
        successful_tests = len([r for r in zkml_results if r.success]) + len([r for r in crypto_results if r.success])
    ...
    def _save_results(self, results: Dict[str, Any]):
        ...
        if results["crypto_results"]:
            ...
            crypto_df.to_csv(self.output_dir / "crypto_benchmark_results.csv", index=False)
    ...
    def _generate_visualizations(
        ...
        if zkml_results:
            ...
            if successful_zkml:
                ...
                success_rates = [framework_success[f]["success"] / framework_success[f]["total"] 
                               for f in frameworks]
                ...
                plt.tight_layout()
                plt.savefig(self.output_dir / "zkml_benchmark_plots.png", dpi=300, bbox_inches='tight')
                plt.close()
        ...
        if crypto_results:
            ...
            if successful_crypto:
                ...
                plt.tight_layout()
                plt.savefig(self.output_dir / "crypto_benchmark_plots.png", dpi=300, bbox_inches='tight')
                plt.close()
...
async def run_phaze_benchmarks(
    output_dir: str = "/tmp/phaze_benchmarks",
