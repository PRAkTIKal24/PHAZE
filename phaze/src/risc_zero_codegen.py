...
class RiscZeroArchitectureRegistry:
    ...
    def __init__(self):
        self.architectures = {}
        self.templates_dir = Path(__file__).parent.parent.parent / "rust_bindings" / "guest_templates"
        self.guest_programs_dir = Path(__file__).parent.parent.parent / "rust_bindings" / "guest_programs"
    ...
    def _load_registry(self):
        ...
        if self.registry_file.exists():
            try:
                ...
                logger.info(f"Loaded {len(self.architectures)} registered architectures")
    ...
    def scan_model_factory(self, dataset_config: Optional[Dict[str, Any]] = None) -> List[Tuple[str, ModelComplexity, Dict[str, Any]]]:
        ...
        logger.info(f"Scanning model factory for dataset: {dataset_config.get('dataset_name', 'custom')}")
        ...
        for architecture in architectures:
            for complexity in complexities:
                try:
                    ...
                    logger.warning(f"Failed to create model {architecture}_{complexity.value}: {e}")
                    continue
        
        return model_configs

    def register_all_factory_models(self, dataset_config: Optional[Dict[str, Any]] = None) -> int:
        ...
        dataset_name = dataset_config.get('dataset_name', 'default') if dataset_config else 'mnist'
        logger.info(f"Registered {newly_registered} new architectures from model factory for dataset: {dataset_name}")
...
'''
    ...
    def _customize_template(self, template_content: str, arch_info: Dict[str, Any]) -> str:
...
class RiscZeroBuildManager:
    ...
    def _build_guest_program(self, guest_dir: Path) -> bool:
        ...
        try:
            ...
            if result.returncode != 0:
                logger.error("cargo-risczero not found. Please install RISC Zero toolchain.")
                return False
            ...
            if result.returncode == 0:
                ...
                elf_path = guest_dir / "target" / "riscv32im-risc0-zkvm-elf" / "release" / guest_dir.name
                if elf_path.exists():
    ...
    def get_guest_program_path(self, arch_key: str) -> Optional[Path]:
        ...
        guest_program_name = arch_info["guest_program_name"]
        guest_dir = self.registry.guest_programs_dir / guest_program_name
        elf_path = guest_dir / "target" / "riscv32im-risc0-zkvm-elf" / "release" / guest_program_name
