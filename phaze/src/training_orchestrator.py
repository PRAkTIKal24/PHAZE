...
class TrainingOrchestrator:
    ...
    def _export_model_to_onnx(
        self, model: nn.Module, model_metadata: Dict[str, Any]
    ) -> Path:
        ...
        try:
            ...
            # Export to ONNX
            # Export to ONNX using legacy exporter for EZKL compatibility
            model.eval()
            with torch.no_grad():
                torch.onnx.export(
                    model,
...
async def run_pipeline_with_pretrained_model(
    config: PHAZEConfig, model_info: Dict[str, Any]
) -> Dict[str, Any]:
    ...
    try:
        ...
        early_exit_models = orchestrator.run_early_exit_generation(
            fake_training_results
        )
        ...
        plot_results = orchestrator.run_plotting_phase(
            fake_training_results, zkml_results, crypto_results
        )
