...
def build_guest_program():
    ...
    try:
        ...
        elf_path = guest_dir / "target" / "riscv32im-risc0-zkvm-elf" / "release" / "risc0_guest"
        if elf_path.exists():
...
def build_rust_bindings():
    ...
    try:
        ...
        try:
            run_command(["uv", "run", "maturin", "--version"], check=True, capture_output=True)
...
def install_python_package(development=True):
    ...
    try:
        ...
        if development:
            ...
            run_command(["uv", "pip", "install", "-e", "."], cwd=project_root, check=True)
            
            # Run dev setup
            print("⚙️ Running development setup...")
            run_command(["uv", "run", "python", "dev_setup.py"], cwd=project_root, check=True)
        else:
...
def verify_installation():
    ...
    try:
        ...
        # Test CLI
        result = run_command(["uv", "run", "phaze", "--help"], capture_output=True, check=True)
...
def main():
    ...
    parser.add_argument("--skip-risc-zero", action="store_true", 
                       help="Skip RISC Zero toolchain installation")
    parser.add_argument("--development", action="store_true", default=True,
                       help="Install in development mode (default)")
    parser.add_argument("--production", action="store_true",
                       help="Install in production mode")
    ...
    if success:
        ...
        print("   uv run python -c \"import rust_zkml_bindings; print('RISC Zero ready!')\"")
    else:
